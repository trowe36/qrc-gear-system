...
const Login = () => {
  ...
  async function validateLogin() {
    let fetchLink = "api/login";
    return fetch(fetchLink, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ user: state.username, pass: state.password }),
    }).then((res) => res.json());
  }

  const handleLogin = () => {
    console.log(
      `entered email:  ${state.username} and password :  ${state.password}`
    );
    //TODO: use username to get hashed password from db. check if match. change code below
    //send username and password to backend.
    //in backend compare passwords encryption etc and return status code to frontend. dispatch the message.
    validateLogin().then((response) => {
      console.log(response.message);
      if (response.message === "Success - Password Match") {
        dispatch({
          type: "loginSuccess",
